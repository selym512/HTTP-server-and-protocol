...
func (r *Request) parseSingle(data []byte) (int, error) {
	...
	case requestStateInitialized:
		reqLine, parsed, err := parseRequestLine(data)
		if err != nil {
			return 0, err
		}
		if parsed == 0 {
	...
	case requestStateParsingBody:
		...
		if err != nil {
			return 0, fmt.Errorf("Content-Length is not a valid Integer")
		}
		r.Body = append(r.Body[:], data[:]...)
		r.bodyLengthRead += len(data)
		if r.bodyLengthRead > contentLength {
			return len(data), fmt.Errorf("Request Body is beyond Content-Length")
		}
		if r.bodyLengthRead == contentLength {
			r.state = requestStateDone
		}
...
func parseRequestLine(data []byte) (*RequestLine, int, error) {
	...
	return requestLine, idx + 2, nil //idx + 2 ?
}
